...
Je.set_axes_args(**arg_2d, title="$J_{\\phi}^e(x, y)$")
Ji.set_axes_args(**arg_2d, title="$J_{\\phi}^i(x, y)$")
Bz.set_axes_args(**arg_2d, title="$B_z(x, y)$")

Je_avg.set_axes_args(title="$\\langle J_{\\phi}^e(x, y) \\rangle_\\phi$ and $J_{\\phi}^e(x, 0)$",
    ylim=(vJe[0], vJe[1]),
    yticks=np.linspace(vJe[0], vJe[1], 5),
    **arg_comm,
)

Ji_avg.set_axes_args(title="$\\langle J_{\\phi}^i(x, y) \\rangle_\\phi$ and $J_{\\phi}^i(x, 0)$",
    ylim=(vJi[0], vJi[1]),
    yticks=np.linspace(vJi[0], vJi[1], 5),
    **arg_comm,
)

Bz_avg.set_axes_args(title="$\\langle B_z(x, y) \\rangle_\\phi$ and $B_z(x, 0)$",
    ylim=(vB[0] - 0.01, vB[1] + 0.01),
    yticks=np.linspace(vB[0], vB[1], 5),
    **arg_comm,
)

res_dir = f"./{params_path}/Currents_phi"
mkdir(res_dir)
...
for t in t_range:
    ...
    _, Je.data = vx_vy_to_vr_va(comp_x, comp_y, COS, SIN)
    ...
    _, Ji.data = vx_vy_to_vr_va(comp_x, comp_y, COS, SIN)
